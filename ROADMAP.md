...
- **Glama (TDQS C 2,4, calculé à 21 h 06 pendant le déploiement, sur 41 outils)** : se recalcule automatiquement (ou bouton "Sync Server", onglet Admin). Leviers restants : (1) ajouter les annotations MCP `readOnlyHint`/`idempotentHint`/`openWorldHint` à tous les outils (tous en lecture seule ; motif de la plupart des "Behavior 3/5") ; (2) renforcer les descriptions les plus faibles (favicon B 3,2, password_check, repo_health_check, csv_json, security_headers : manque de "quand l'utiliser") -- la note serveur pèse 40 % sur l'outil le plus faible ; (3) cohérence (nommage 2/5, nombre d'outils 2/5) = structurel, lié au renommage non fait.
- Registre MCP officiel (`io.github.presendapp/presend-mcp`) : sa description dit encore "41 security & dev API tools" (reprise telle quelle par Glama) -- republier avec 40.
