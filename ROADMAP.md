...
- Resynchroniser la collection Postman (POST batch des 2 endpoints + endpoints récents). GitHub Packages `@presendapp/presend-api` : dernière version constatée = {"message":"You need at least read:packages scope to get a package's versions.","documentation_url":"https://docs.github.com/rest/packages/packages#list-package-versions-for-a-package-owned-by-a-user","status":"403"}inconnu (droit read:packages manquant ?) -- décider si on la maintient (ajouter une étape au workflow) ou si on la retire de la doc.
- `tx-decode` : pas d'exemple de paramètre dans openapi.json (test du client npm laissé en commentaire) -- ajouter un exemple de transaction réelle.
