...
- [x] **25 sept. -- qualité MCP, 1re passe (non cassante hormis `ip`)** : 40 descriptions réécrites (moyenne 197 caractères au lieu de ~38 : rôle, quand préférer un outil voisin, limites), vérifiées sur des réponses réelles de l'API ; `ip` retiré (résultat faux via MCP) ; `phone_verify` ne devine plus le pays ; `user_agent` exige `ua` ; serveur v3.0.0. NON fait : renommage/regroupement des outils (cassant pour les intégrations existantes, à décider). Suites : relancer l'analyse Glama (TDQS était C 2,6/5) ; mettre à jour le compteur 41 -> 40 dans la PR punkpeye/awesome-mcp-servers #14651 ; vérifier si la fiche du registre MCP officiel affiche un nombre d'outils.
