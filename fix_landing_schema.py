...
# Mapping des outils vers leurs infos
TOOL_INFO = {
    "remove-exif-iphone": {
...
# Default fallback for unknown landings
DEFAULT_INFO = {
    "name": "Presend Tool",
...
def get_landing_key(filename):
    """Extract the landing key from filename like 'remove-exif-iphone.html'"""
    return filename.replace('.html', '')

def get_tool_url(landing_key):
    """Map landing to main tool URL"""
    mappings = {
...
def process_landing(filepath):
    ...
    tool_path = f"/tools/{tool_url}" if tool_url else ""
    ...
    # SoftwareApplication
    sw_app = build_software_app_schema(info, landing_url, tool_url)
    if sw_app.get("isRelatedTo") is None:
        del sw_app["isRelatedTo"]
    schemas.append(json.dumps(sw_app, indent=2, ensure_ascii=False))
    
    # BreadcrumbList (replace existing incomplete one)
    breadcrumb = build_breadcrumb(landing_url, info["name"])
    schemas.append(json.dumps(breadcrumb, indent=2, ensure_ascii=False))
    
    # FAQPage
    faq = build_faq_schema(info)
    schemas.append(json.dumps(faq, indent=2, ensure_ascii=False))
    
    # Build the new schema block
    schema_block = "\n".join([
        f'<script type="application/ld+json">\n{s}\n</script>'
        for s in schemas
    ])
    
    # Remove existing schema.org scripts (Organization, BreadcrumbList, etc.)
    ...
    # Also remove the <!--seo-org--> and <!--seo-breadcrumb--> comments if present
    content = re.sub(r'<!--seo-[^>]*-->', '', content)
    
    # Insert new schema block before </head>
    content = content.replace('</head>', f'{schema_block}\n</head>')
    
    # Also update the BreadcrumbList in body if it exists inline (some pages have it)
    # Remove any remaining inline schema blocks
    content = re.sub(
