...
def process_landing(filepath):
    ...
    # Step 2: Insert SoftwareApplication BEFORE the first ld+json script
    sw_app_json = json.dumps(build_software_app_schema(info, landing_url, tool_url), indent=2, ensure_ascii=False)
    sw_app_block = f'<script type="application/ld+json">\n{sw_app_json}\n</script>\n'
    ...
    if first_script_match:
        ...
        content = content[:insert_pos] + sw_app_block + content[insert_pos:]
        sw_inserted = True
    else:
        sw_inserted = False
    
    # Step 3: Insert FAQPage AFTER the last ld+json script (but before </head>)
    faq_json = json.dumps(build_faq_schema(info), indent=2, ensure_ascii=False)
    faq_block = f'\n<script type="application/ld+json">\n{faq_json}\n</script>'
    
    head_match = re.search(r'(<head>.*?</head>)', content, re.DOTALL)
    if head_match:
        head_content = head_match.group(1)
        # Find last ld+json script in head
    ...
    status = "✓" if (breadcrumb_replaced and sw_inserted and faq_inserted) else "⚠"
    print(f"{status} {filename} — breadcrumb={'replaced' if breadcrumb_replaced else 'NO'}, sw={'yes' if sw_inserted else 'no'}, faq={'yes' if faq_inserted else 'no'}")
